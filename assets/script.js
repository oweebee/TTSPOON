...
async function saveFiles(fix_filename, blob, from_ind, to_ind) {
	console.log('[saveFiles] début, dossier:', parts_book[from_ind].my_filename, 'fichier:', fix_filename, 'taille blob:', blob.size)
	const new_folder_handle = await save_path_handle.getDirectoryHandle(parts_book[from_ind].my_filename, { create: true });
	console.log('[saveFiles] dossier créé/ouvert OK')
	const fileHandle = await new_folder_handle.getFileHandle(fix_filename, { create: true });
	console.log('[saveFiles] fichier créé/ouvert OK')
	const writableStream = await fileHandle.createWritable();
	const writable = writableStream.getWriter();
	await writable.write(blob);
	await writable.close();
	console.log('[saveFiles] écriture terminée avec succès')
	...
	if (parts_book[from_ind].start_save == false) {
		...
		try {
			...
			if (save_path_handle ?? false) {
				saveFiles(fix_filename, blob_mp3, from_ind, to_ind).catch(err => {
					console.error('Échec de l’écriture sur le disque (fusion) :', err)
					stat_info.textContent = 'Erreur d’enregistrement (voir console) : ' + (err && err.message ? err.message : err)
				})
			} else {
			...
			// Erreur synchrone (ex: mémoire insuffisante pour assembler un très gros fichier fusionné)
			console.error('Échec de la fusion des MP3 :', err)
			stat_info.textContent = 'Erreur de fusion (voir console) : ' + (err && err.message ? err.message : err)
		}
	}
}
...
function do_marge() {
	...
	console.log('[do_marge] books_map:', JSON.stringify(books_map), 'save_path_handle set?', !!save_path_handle)

	if (books_map.length > 0) {
		for (let book_map of books_map) {
			if (book_map[0] == true && book_map[3] > 0) {
				console.log('[do_marge] -> save_merge appelé pour', book_map)
